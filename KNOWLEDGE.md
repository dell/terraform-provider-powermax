...
### What does it do?
Terraform provider for Dell PowerMax enterprise storage arrays. Exposes 8 resources and 9 data sources covering volumes, storage groups, port groups, host groups, hosts, masking views, snapshots, and SRDF pairs
...
8 resources and 9 data sources covering volumes, storage groups, port groups, host groups, hosts, masking views, snapshots, and SRDF pairs. Resources use `*_resource.go` naming under `powermax/provider/`.
...
leave orphaned resources on the PowerMax array. These must be
...
**API rate limiting:** PowerMax arrays may enforce API rate
...
PowerMax API endpoint simultaneously can cause contention.
